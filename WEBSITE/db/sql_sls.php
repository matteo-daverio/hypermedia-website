...
if (!empty($_GET['categoria'])){         //Called by the promotion page (promozione_sls.html)
    ...
    $rows = $db -> select("SELECT `id` , `titolo` , `promozione`, `attivabile` , `des_offerta` , `img_pres` FROM `sls` where categoria = " . $categ  );
}

else if($_GET['prom'] === "true"){  //Called by the promotion page (promozione_sls.html)
    $rows = $db -> select("SELECT `id` , `titolo` , `promozione`, `attivabile` , `categoria` , `des_offerta` , `img_pres` FROM `sls`  WHERE  promozione = '1' "); 
}
    ...
    $id = $db -> quote($_GET['id']);
    $rows = $db -> select("SELECT `id`, `titolo` , `categoria`, `promozione`, `img_land`,  `img_prod`, `img_pres`, `des_offerta` , `des_specifiche`, `act_act`,  `act_device_correlati_nome`, `act_device_correlati_link`, `act_timricorda` FROM `sls` WHERE id = " . $id );   
}
